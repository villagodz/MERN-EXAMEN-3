...
const EditLibro = () => {
    ...
    const handleUpdateLibro = async () => {
        try {
            ...
            const response = await axios.put(`/api/libros/${id}`, libro);
            const data = response.data;
            setLibro(data);
            navigate("/app/home")
        } catch (error) {
    ...
    const handleDeleteLibro = async () => {
        try {
            ...
            const response = await axios.delete(`/api/libros/${id}`);
            const data = response.data;
            console.log(data);
            navigate("/app/home")
        } catch (error) {
        ...
        <>
            <Container 
                ...
                <Stack sx={{ width: "100%" }} spacing={2}>
                    ...
                    />
                    <Button variant="contained" color="primary" onClick={handleUpdateLibro}>Agregar</Button>
                    <Button variant="contained" color="error" onClick={handleDeleteLibro}>Eliminar</Button>
                </Stack>
            </Container>
        </>
    )
